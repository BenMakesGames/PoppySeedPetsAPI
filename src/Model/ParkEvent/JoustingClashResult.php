...
class JoustingClashResult
{
    ...
    public function __construct(JoustingTeam $team1, JoustingTeam $team2)
    {
        ...
        $rider1Attack = mt_rand(1, 10 + $rider1Skills->getDexterity() + $rider1Skills->getBrawl());
        $rider1Dodge = mt_rand(1, 10 + $rider1Skills->getDexterity() + $rider1Skills->getBrawl());

        $rider2Attack = mt_rand(1, 10 + $rider2Skills->getDexterity() + $rider2Skills->getBrawl());
        $rider2Dodge = mt_rand(1, 10 + $rider2Skills->getDexterity() + $rider2Skills->getBrawl());
        ...
        if($this->rider1Hit)
        {
            $rider1HitStrength = mt_rand(1, 10 + $rider1Skills->getStrength() + $totalMountSpeed);

            $this->rider1BrokeLance = $rider1HitStrength >= 8;

            $rider2BraceRoll = mt_rand(1, 10 + $rider2Skills->getStamina() * 2 + $rider2Skills->getDexterity());
            $mount2BraceRoll = mt_rand(1, 10 + $mount2Skills->getDexterity() * 2 + $mount2Skills->getStamina());
        ...
        if($this->rider2Hit)
        {
            $rider2HitStrength = mt_rand(1, 10 + $rider2Skills->getStrength() + $totalMountSpeed);

            $this->rider2BrokeLance = $rider2HitStrength >= 8;

            $rider1BraceRoll = mt_rand(1, 10 + $rider1Skills->getStamina() * 2 + $rider1Skills->getDexterity());
            $mount1BraceRoll = mt_rand(1, 10 + $mount1Skills->getDexterity() * 2 + $mount1Skills->getStamina());
