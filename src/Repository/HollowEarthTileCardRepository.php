<?php

namespace App\Repository;

use App\Entity\HollowEarthTileCard;
use Doctrine\Bundle\DoctrineBundle\Repository\ServiceEntityRepository;
use Doctrine\Persistence\ManagerRegistry;

/**
 * @method HollowEarthTileCard|null find($id, $lockMode = null, $lockVersion = null)
 * @method HollowEarthTileCard|null findOneBy(array $criteria, array $orderBy = null)
 * @method HollowEarthTileCard[]    findAll()
 * @method HollowEarthTileCard[]    findBy(array $criteria, array $orderBy = null, $limit = null, $offset = null)
 * @deprecated
 */
class HollowEarthTileCardRepository extends ServiceEntityRepository
{
    public function __construct(ManagerRegistry $registry)
    {
        parent::__construct($registry, HollowEarthTileCard::class);
    }

    // /**
    //  * @return HollowEarthTileCard[] Returns an array of HollowEarthTileCard objects
    //  */
    /*
    public function findByExampleField($value)
    {
        return $this->createQueryBuilder('h')
            ->andWhere('h.exampleField = :val')
            ->setParameter('val', $value)
            ->orderBy('h.id', 'ASC')
            ->setMaxResults(10)
            ->getQuery()
            ->getResult()
        ;
    }
    */

    /*
    public function findOneBySomeField($value): ?HollowEarthTileCard
    {
        return $this->createQueryBuilder('h')
            ->andWhere('h.exampleField = :val')
            ->setParameter('val', $value)
            ->getQuery()
            ->getOneOrNullResult()
        ;
    }
    */
}
