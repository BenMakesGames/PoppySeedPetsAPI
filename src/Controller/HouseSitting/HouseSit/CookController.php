...
class CookController extends AbstractController
{
    ...
    public function cook(
        int $houseSitForId, Request $request, EntityManagerInterface $em, IRandom $rng,
        ResponseService $responseService, CookingService $cookingService
    )
    {
        /** @var User $user */
        $user = $this->getUser();
        ...
        if(ArrayFunctions::any($inventory, fn(Inventory $i) => $i->getLocation() !== LocationEnum::HOME))
            throw new PSPFormValidationException('All of the items must be in the house.');
        ...
        $results = $cookingService->prepareRecipeByHand($user, $inventory[0]->getOwner(), $inventory);
        ...
        if($totalQuantity < 4)
            ...
            $exclaim = '! (' . $rng->rngNextFromArray([ 'Dang!', 'Wow!', 'Incredible...', 'So cook! Very meal!', 'A veritable feast!', 'Such skill! Such finesse!' ]) . ')';
