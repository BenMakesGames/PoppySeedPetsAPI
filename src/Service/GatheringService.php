...
class GatheringService
{
    ...
    {
        $this->activityLogService->createActivityLog($pet, $pet->getName() . ' went out gathering, but couldn\'t find anything.');
    ...
    private function foundBerryBush(Pet $pet)
    {
        ...
        {
            $this->activityLogService->createActivityLog($pet, $pet->getName() . ' harvested berries from a Thorny Berry Bush.');
        }
        else
        {
            $this->activityLogService->createActivityLog($pet, $pet->getName() . ' got scratched up harvesting berries from a Thorny Berry Bush.');
    ...
    private function foundHollowLog(Pet $pet)
    {
        if(\mt_rand(1, 4) === 1)
        {
            ...
            {
                $this->activityLogService->createActivityLog($pet, $pet->getName() . ' found a Huge Toad inside a Hollow Log, got the jump on it, wrestled it to the ground, and claimed its Toadstool!');
            ...
            {
                $this->activityLogService->createActivityLog($pet, $pet->getName() . ' found a Huge Toad inside a Hollow Log, but it got away!');
        ...
        {
            $this->activityLogService->createActivityLog($pet, $pet->getName() . ' broke a Crooked Branch off of a Hollow Log.');
    ...
    private function foundBirdNest(Pet $pet)
    {
        ...
        {
            $this->activityLogService->createActivityLog($pet, $pet->getName() . ' stole an Egg from a Bird Nest.');
        ...
        else
        {
            ...
            {
                $this->activityLogService->createActivityLog($pet, $pet->getName() . ' tried to steal an Egg from a Bird Nest, was spotted by a parent, and was able to defeat it in combat!');
            ...
            {
                $this->activityLogService->createActivityLog($pet, $pet->getName() . ' tried to steal an Egg from a Bird Nest, but was spotted by a parent, and chased off!');
