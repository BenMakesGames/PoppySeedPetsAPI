    ...
    public function createNonsenserang(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        if($roll <= 3)
        {
            ...
            if(mt_rand(1, 2) === 1)
            {
                ...
                return $this->responseService->createActivityLog($pet, $pet->getName() . ' tried to make another boomerang blade for a Nonsenserang, but the base plate of the 3D Printer moved, jacking up the Plastic :(', '');
            }
            else
            {
                $this->inventoryService->loseItem('Plastic Boomerang', $pet->getOwner(), LocationEnum::HOME, 1);
                $this->petExperienceService->gainExp($pet, 1, [ PetSkillEnum::SCIENCE, PetSkillEnum::CRAFTS ]);
                $activityLog = $this->responseService->createActivityLog($pet, $pet->getName() . ' tried to weld two Plastic Boomerangs together, but warped one of them beyond repair :( Well, at least it\'s still useful as a chunk of Plastic...', '');
        ...
        else if($roll >= 14)
        {
            ...
            $pet->increaseEsteem(2);
            $activityLog = $this->responseService->createActivityLog($pet, $pet->getName() . ' created a Nonsenserang!', '')
