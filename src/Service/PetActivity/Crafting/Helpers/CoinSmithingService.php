...
class CoinSmithingService
{
    ...
    public function makeSilverCoins(ComputedPetSkills $petWithSkills, Item $triedToMake): PetActivityLog
    {
        ...
        $activityLog = PetActivityLogFactory::createUnreadLog($this->em, $pet, '%pet:' . $pet->getId() . '.name% tried to forge ' . $triedToMake->getNameWithArticle() . ' from a Silver Bar, but spilled some of the silver, and almost burned themselves! They used the leftovers to make ' . $moneys . '~~m~~ worth of silver coins, instead.')
        ...
        PetBadgeHelpers::awardBadge($this->em, $pet, PetBadgeEnum::MintedMoneys, $activityLog);
    ...
    public function makeGoldCoins(ComputedPetSkills $petWithSkills, Item $triedToMake): PetActivityLog
    {
        ...
        $activityLog = PetActivityLogFactory::createUnreadLog($this->em, $pet, '%pet:' . $pet->getId() . '.name% tried to forge ' . $triedToMake->getNameWithArticle() . ' from a Gold Bar, but spilled some of the gold, and almost burned themselves! They used the leftovers to make ' . $moneys . '~~m~~ worth of gold coins, instead.')
            ->setIcon('icons/activity-logs/moneys')
            ->addTags(PetActivityLogTagHelpers::findByNames($this->em, [ 'Smithing', 'Moneys' ]))
        ;
