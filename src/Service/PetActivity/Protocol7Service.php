...
class Protocol7Service
{
    ...
    private function doTimesArrow(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        if($roll >= 15)
        {
            ...
            [$locationAndAction, $actioning] = ArrayFunctions::pick_one([
    ...
    private function deliverMessagesForCorrespondence(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        return $this->responseService->createActivityLog($pet, '%pet:' . $pet->getId() . '.name% accessed Project-E. Correspondence had some message-delivery jobs, so ' . $pet->getName() . ' picked a couple up, earning ' . $moneys . '~~m~~ for their trouble.', '');
    }
    ...
    private function encounterAnnabellastasia(Pet $pet): PetActivityLog
    {
        ...
        $activityLog = $this->responseService->createActivityLog($pet, 'In Project-E, ' . '%pet:' . $pet->getId() . '.name% ran into a girl named Annabellastasia, who handed ' . $pet->getName() . ' a Black Bow.', 'items/hat/bow-black')
    ...
    private function encounterGarbageCollector(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        if($success)
        {
            ...
            if($pet->isInGuild(GuildEnum::TIMES_ARROW))
            {
                ...
                if($pet->hasMerit(MeritEnum::SOOTHING_VOICE) && mt_rand(1, 3) === 1)
                    $logMessage = $pet->getName() . ' met with a Garbage Collector in Project-E. Happy to help a member of Time\'s Arrow - especially one with such a Soothing Voice! - it handed over a Pointer.';
                else
                    $logMessage = $pet->getName() . ' met with a Garbage Collector in Project-E. Happy to help a member of Time\'s Arrow, it handed over a Pointer.';
    ...
    private function foundLayer02(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        {
            $this->petExperienceService->gainExp($pet, 1, [ PetSkillEnum::SCIENCE ]);
            return $this->responseService->createActivityLog($pet, '%pet:' . $pet->getId() . '.name% accessed Layer 02 of Project-E, but ' . $baddie . ' hijacked their session.', '');
        }
    }
    ...
    private function foundProtectedSector(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        else
        {
            ...
            $this->petExperienceService->gainExp($pet, 1, [ PetSkillEnum::SCIENCE ]);
            return $this->responseService->createActivityLog($pet, '%pet:' . $pet->getId() . '.name% tried to access a protected sector of Project-E, but couldn\'t get elevated permissions.', '');
        }
    }
    ...
    private function exploreInsecurePort(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        else
        {
            $pet->increaseSafety(-1);
            $this->petExperienceService->gainExp($pet, 1, [ PetSkillEnum::SCIENCE ]);
            return $this->responseService->createActivityLog($pet, '%pet:' . $pet->getId() . '.name% accessed an insecure port in Project-E, but their service was disrupted by ' . $baddie . '.', '');
        }
    }
    ...
    private function repairShortedCircuit(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        else if(mt_rand(1, max(10, 50 - $pet->getSkills()->getIntelligence())) === 1)
        {
            ...
            $activityLog = $this->responseService->createActivityLog($pet, '%pet:' . $pet->getId() . '.name%\'s line was suddenly shorted while they were exploring Project-E. ' . $pet->getName() . ' managed to capture some Lightning in a Bottle before being forcefully disconnected!', '');
        ...
        else
        {
            ...
            $activityLog = $this->responseService->createActivityLog($pet, '%pet:' . $pet->getId() . '.name%\'s line was suddenly shorted while they were exploring Project-E. ' . $pet->getName() . ' managed to grab a couple Pointers before being forcefully disconnected.', '');
    ...
    private function foundCorruptSector(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        else
        {
            ...
            else if(mt_rand(1, 100) === 1 && $pet->hasMerit(MeritEnum::LUCKY))
            {
                $otherLoot = 'Recovered Archive';
                $lucky = true;
            }
