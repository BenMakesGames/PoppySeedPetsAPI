...
class Protocol7Service
{
    ...
    private function doTimesArrow(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        else
        {
            ...
            return $this->responseService->createActivityLog($pet, $pet->getName() . ' accessed Project-E, following some breadcrumbs left by other members of Time\'s Arrow, but they lost the trail, and weren\'t able to find it again.', 'icons/activity-logs/confused');
        }
    }
    ...
    private function encounterGarbageCollector(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        else
        {
            ...
            if($pet->isInGuild(GuildEnum::TIMES_ARROW))
                return $this->responseService->createActivityLog($pet, $pet->getName() . ' met with a Garbage Collector in Project-E. It was happy to help a member of Time\'s Arrow, but didn\'t have anything at the moment.', '');
            else
    ...
    private function exploreWalledGarden(ComputedPetSkills $petWithSkills): PetActivityLog
    {
        ...
        if(mt_rand(1, 100) === 1)
        {
            ...
            $activityLog = $this->responseService->createActivityLog($pet, $pet->getName() . ' ' . $snuck . ' into a Walled Garden, but ran into a Pirate doing the same! ' . $pet->getName() . ' defeated the Pirate, stole its Jolliest Roger, and ran off before the Walled Garden\'s security system detected them! Yarr!', '');
